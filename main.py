...
flags.DEFINE_integer("eval_every", 1, "Run validation every N epochs")
flags.DEFINE_string("path", "./", "The path where the data is")
flags.DEFINE_string("output", "./output", "The path to store output message")
...
flags.DEFINE_multi_string(
    "relation", ["buy", "wanted", "clicked", "detail_view"], "Relations"
)
# pretrain model flags
flags.DEFINE_bool("pretrain_frozen", "False", "Froze the pretrain parameter or not")
flags.DEFINE_string(
    "create_embeddings", "True", "Pretrain or not? If not create embedding here!"
)
flags.DEFINE_string(
    "pretrain_path",
    "/data3/jinbowen/multi_behavior/output/Steam/MF-Steam-lr3-L4@jinbowen",
    "Path where the pretrain model is.",
)
# graph based
...
# Ours
flags.DEFINE_float(
    "lamb", 0.1, "Lambda for the loss for MultiGNN with item space calculation"
)
